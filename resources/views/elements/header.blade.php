        ...
        ***********************************-->
        <div class="nav-header">
            <a href="{{route('home')}}" class="brand-logo">
                ...
                <h4 class="text-primary font-w600 mb-0">
					@if(!empty(Auth::user()->store_id))
						@php
							$result = \App\Models\Store::where(['id' => Auth::user()->store_id ],'status',1)->first();	
						@endphp
                        <img class="logo-abbr" src="{{asset('public/images/stores/'). '/' .$result->image }}" alt="{{$result->name}}">	
						{{$result->name }}	
					@else
